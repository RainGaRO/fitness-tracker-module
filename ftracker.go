...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	averageSpeedMeters := meanSpeed(action, duration) * kmhInMsec
	speedSquared := math.Pow(averageSpeedMeters, 2)
	heightMeters := height / cmInM
	return ((walkingCaloriesWeightMultiplier*weight + (speedSquared/heightMeters)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
